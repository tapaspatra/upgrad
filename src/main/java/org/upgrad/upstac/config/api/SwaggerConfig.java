...
public class SwaggerConfig {
	...
	public Docket api() {
		return new Docket(DocumentationType.SWAGGER_2).apiInfo(apiInfo()).select().apis(RequestHandlerSelectors.any())
				.paths(pathsToBeDocumented()).build().securitySchemes(getSecuritySchemes())
	...
	private List<SecurityContext> getSecurityContexts() {
		SecurityContext context = SecurityContext.builder().securityReferences(getSecurityReferences())
				.forPaths(pathsToBeSecured()).build();
	...
	private Predicate<String> pathsToBeSecured() {
		return or(ant("/api/testrequests/**"), ant("/api/government/**"), ant("/api/consultations/**"),
				ant("/users/**"), ant("/api/labrequests/**")

		);
	}

	private ApiInfo apiInfo() {
		return new ApiInfoBuilder().title("Upgrad UPSTAC System").description("UPSTAC Track APIs").contact("Upgrad")
